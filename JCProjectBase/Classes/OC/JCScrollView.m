...
@interface JCScrollView () <UIScrollViewDelegate> {
...
- (JCSubView *)dequeueReuseCellWithIdentifier:(NSString *)identifier{
    for (JCSubView *sub in self.reuseViewAry) {
...
- (void)setSubView{
    ...
    self.scrollView.frame = CGRectMake(11, 0, _subViewWith, self.frame.size.height);
...
- (void)checkOffset {
    [self updateFrame:self.scrollView];
    //    self.scrollView.contentOffset = _point;
    [self.scrollView setContentOffset:_point animated:false];
...
- (void)refreshCell{
    ...
    for (int i = 0; i<5; i++) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(scrollView:subViewFrame:cellAtIndex:)]) {
            ...
            for (JCSubView *sub in subView.subviews) {
                sub.isUser = NO;
                [self.userViewAry removeObject:sub];
                [self.reuseViewAry addObject:sub];
                [sub removeFromSuperview];
            }
            JCSubView *sub_view = [self.delegate scrollView:self subViewFrame:_subFrame cellAtIndex:index];
...
- (void)updateOffset:(UIScrollView *)scrollView {
    ...
    if (_index == 0 && contentOffsetX < _point.x && _isEnableMargin) {
        [scrollView setContentOffset:_point animated:false];
...
- (void)updateFrame:(UIScrollView *)scrollView {
    __block CGFloat w = _subViewWith;
    CGFloat t = 0.4;
    if (_index == 0) {
        if (scrollView.frame.origin.x != 11) {
            
            [UIView animateWithDuration:t animations:^{
                self.scrollView.frame = CGRectMake(11, 0, w, self.frame.size.height);
            }];
        }
    }else{
        if (scrollView.contentOffset.x != (self.frame.size.width-_subViewWith)/2) {
            [UIView animateWithDuration:t animations:^{
                self.scrollView.frame = CGRectMake((self.frame.size.width-w)/2, 0, w, self.frame.size.height);
            }];
        }
    }
}
...
- (void)setScrollViewType:(ScrollType)scrollViewType{
    if (scrollViewType == JCScrollViewRepeat) {
